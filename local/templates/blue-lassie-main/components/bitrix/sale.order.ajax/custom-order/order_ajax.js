...
//--------------------------------------------------------------------------EDIT PROPS ITEM --------------------------------------------------------------------------------------------------------
        ...
        getPropertyRowNode: function(property, propsItemsContainer, disabled) {
            ...
            if (disabled) {
                ...
                BX.addClass(propsItemNode, "form__row"); //"form-group bx-soa-customer-field");
				...
				/*---------------------------------------------------------------------------ФОРМИРУЕМ LABLE*/
                label =  BX.create('DIV', 
					{props: { className: 'form__col form__col_width_220'},
					children: [ 
						BX.create('LABEL',  {                 
							attrs: { 'for': 'soa-property-' + property.getId() },
							props: { className: 'form__label' }, 	//'bx-soa-custom-label' },
							html: textHtml
			})]});
        ...
        insertStringProperty: function(property, propsItemNode, disabled) {              //--------------------------------------add TEXT INPUT
            var prop, inputs, values, i, propContainer, inputWidth = 260;
			
			if(property.getName() == 'Фамилия, имя, отчество' ){                        //--------------------set input width
				inputWidth = 375;
			}else if(property.getName() == 'Электронная почта' || property.getName() == 'Контактный телефон' || property.getName() == 'Город' || property.getName() == 'Улица' || property.getName() == 'Почтовый индекс'){
				inputWidth = 260;
			}else if(property.getName() == 'Номер дома' || property.getName() == 'Корпус' || property.getName() == 'Номер квартиры' ){
				inputWidth = 90;
			}
            if (disabled) {
                prop = this.propsHiddenBlockNode.querySelector('div[data-property-id-row="' + property.getId() + '"]');
                ...
                propContainer = BX.create('DIV', { props: { className: ('form__col form__col_width_' + inputWidth)} });		//'soa-property-container' } });
                property.appendTo(propContainer);
                propsItemNode.appendChild(propContainer);
                this.alterProperty(property.getSettings(), propContainer);
                this.bindValidation(property.getId(), propContainer);
...
//---------------------------------------------- PROPS MAP------------------------------------------------------------------------------------------------------------
        ...
        editPropsComment: function(propsNode) {
            ...
            label = BX.create('LABEL', {
                ...
                props: { className: "form__label"}, //'bx-soa-customer-label' },
            ...
            input = BX.create('TEXTAREA', {
                props: {
                    id: 'orderDescription',
                    cols: '4',
                    className: 'input', //'form-control bx-soa-customer-textarea bx-ios-fix',
