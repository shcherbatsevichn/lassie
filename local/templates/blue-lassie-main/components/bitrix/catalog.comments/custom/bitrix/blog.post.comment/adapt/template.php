			...
			if($comment["SHOW_AS_HIDDEN"] == "Y" || $comment["PUBLISH_STATUS"] == BLOG_PUBLISH_STATUS_PUBLISH || $comment["SHOW_SCREENNED"] == "Y" || $comment["ID"] == "preview")
			{
				...
				if($comment["PUBLISH_STATUS"] == BLOG_PUBLISH_STATUS_PUBLISH || $comment["SHOW_SCREENNED"] == "Y" || $comment["ID"] == "preview")
				{
					...
					<div class="blog-comment-info">
						...
						{
							?>
							<!--<div class="blog-author"><?=$comment["AuthorName"]?></div>-->  <!-- Name -->
							<?
						}
						...
						?>
						<!--<div class="blog-comment-date"><?=$comment["DateFormated"]?></div>-->   <!-- addData -->
					</div>
					<div class="blog-clear-float"></div>
					<div class="blog-comment-content">           <!-- Контентная область -->
						...
						{	
							...
							$addClassArray[$fotmatText[1]] = 'raiting__star_active';
							
							?>
							<div class="raiting review__raiting">
								<span class="raiting__star <?=$addClassArray[5]?>">5</span>
								<span class="raiting__star <?=$addClassArray[4]?>">4 </span>
								<span class="raiting__star <?=$addClassArray[3]?>">3 </span>
								<span class="raiting__star <?=$addClassArray[2]?>">2 </span>
								<span class="raiting__star <?=$addClassArray[1]?>">1 </span>
							</div>
...
//							dbg showComment?
		...
		if($arResult["is_ajax_post"] != "Y" && $arResult["CanUserComment"])
		{
			...
			<div id="form_comment_" >
				...
				<div class="blog-comment-fields" style="background:none;">
					<?
					...
					if($arResult['userID'] == null && $arParams['USER_CONSENT'] == 'Y')
					{
						$APPLICATION->IncludeComponent(
							...
							array(
								...
								"ORIGINATOR_ID" => "",
								"REPLACE" => array(
									'button_caption' => GetMessage("B_B_MS_SEND_CUSTOM"),
